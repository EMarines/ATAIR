	...
	function findPropertiesForFilter() {
		...
		if (reqFiltro.selecTO) {
			filteredList = filteredList.filter(
				(item) => convertOperation(item.selecTO) === reqFiltro.selecTO
			);
		}
...
<div class="container">
	...
	<div class="download-section">
		...
		{#if downloadStatus}
			<div
				...
				class:success={downloadStatus.includes('✅')}
				class:error={downloadStatus.includes('❌')}
			>
				{downloadStatus}
			</div>
		{/if}
	...
	<div class="buttons">
		<Button element="button" type="button" variant="solid" name="Buscar" on:click={cleanSearch}
			>Limpiar busqueda
		</Button>

		<Button element="button" type="button" variant="danger" on:click={onCancel}>Cancelar</Button>
	</div>
...
<style>
	...
	.properties__contanier .title {
		/* Estilo para el subtítulo de propiedades encontradas */
		grid-column: 1 / -1;
		font-size: 1.2rem;
		color: #555;
		margin-bottom: 1rem;
	...
	p {
		/* Estilo para el mensaje de "No se encontraron propiedades" */
		text-align: center;
		color: #777;
		font-size: 1rem;
		margin-top: 2rem;
