<script lang="ts">
  ...
  async function analyzeData() {
    ...
    try {
      ...
      console.log('📝 IDs de contactos encontrados:', Array.from(contactIds).slice(0, 5), '... (total:', contactIds.size, ')');
      
      // Mostrar algunos ejemplos de binnacles para debug
      console.log('📝 Ejemplos de binnacles:', binnacles.slice(0, 3).map(b => ({ id: b.id, to: b.to, action: b.action })));
      ...
      stats = {
        ...
        percentageToDelete: binnacles.length > 0 ? Math.round((orphanedBinnacles.length / binnacles.length) * 100) : 0
      };
  ...
  async function executeCleanup() {
    if (!confirm(`¿Estás seguro de que quieres eliminar ${orphanedBinnacles.length} binnacles huérfanas?\n\nEsta acción NO se puede deshacer (aunque tienes el backup).`)) {
      return;
    }
  ...
  async function restoreFromBackup() {
    ...
    if (!confirm(`¿Estás seguro de que quieres restaurar las binnacles desde el backup?\n\nEsto agregará todas las binnacles del backup a la base de datos actual.`)) {
      return;
    }
...
<main class="cleanup-container">
  <header>
    ...
    <div class="info-note">
      <p><strong>📝 Nota:</strong> A partir de ahora, cuando elimines un contacto, su bitácora se borrará automáticamente. Esta herramienta es para limpiar registros históricos huérfanos.</p>
    </div>
  </header>
      ...
      {:else}
        <div class="results-section">
              ...
              <div class="list-container">
                {#each orphanedBinnacles.slice(0, 10) as binnacle}
                  <div class="orphaned-item">
                    <strong>ID:</strong> {binnacle.id}<br>
                    <strong>ContactID (to):</strong> {binnacle.to || 'Sin contacto asociado'}<br>
                    <strong>Fecha:</strong> {binnacle.date ? new Date(binnacle.date).toLocaleString() : 'Sin fecha'}<br>
                    <strong>Acción:</strong> {binnacle.action || 'Sin acción'}<br>
                    <strong>Comentario:</strong> {(binnacle.comment || '').substring(0, 50)}...
                  </div>
                {/each}
          ...
          <div class="restore-section">
            ...
            <p class="restore-info">Si necesitas restaurar las binnacles desde un backup anterior:</p>
            ...
            <div class="file-input-container">
              ...
              <label for="backup-file" class="file-label">
                {selectedFile ? `Archivo: ${selectedFile.name}` : '📁 Seleccionar archivo de backup (.json)'}
              </label>
            </div>
