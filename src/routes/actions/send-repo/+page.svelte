<script lang="ts">
	...
	function updateVendorProperties() {
		...
		vendors = vendors.filter(
			(vendor) => vendor.id && vendorProperties[vendor.id] && vendorProperties[vendor.id].length > 0
		);
	...
	function toggleSelectAll(event: Event) {
		...
		if (target.checked) {
			...
			selectedVendors = vendors.filter((vendor) => vendor.id).map((vendor) => vendor.id as string);
		} else {
			// Deseleccionar todos
			selectedVendors = [];
		}
	...
	$: allSelected =
		vendors.length > 0 && vendors.filter((v) => v.id).length === selectedVendors.length;
	...
	async function sendToNextVendor() {
		...
		if (!isSending) {
			...
			console.log(
				`🚀 Comenzando envío individual a ${selectedVendorObjects.length} propietarios seleccionados`
			);
		}
		...
		const vendorFullName =
			`${capitalize(vendor.name)} ${vendor.lastname ? capitalize(vendor.lastname) : ''}`.trim();
		console.log(
			`💬 [${currentVendorIndex + 1}/${selectedVendorObjects.length}] Procesando propietario: ${vendorFullName}`
		);
		...
		console.log(
			`📋 Preparando reporte con ${vendorProps.length} propiedades para ${vendorFullName}`
		);
...
- Errores: Ninguno`);
			...
			console.warn(
				`⚠️ Proceso completado con algunos errores. ${enviadosExito} exitosos, ${conError} errores`
			);
...
${conError > 0 ? `\nDetalle de errores:\n${errorsText}` : ''}`);
	...
	async function sendWhatsAppToVendor(vendor: Contact, properties: Property[]) {
		...
		console.log(
			`✅ Ventana de WhatsApp abierta para ${vendor.name} - Espera a que se envíe el mensaje y ciérrala cuando estés listo`
		);
	...
	function createWhatsAppMessage(vendor: Contact, properties: Property[]) {
		...
		if (customMessageTemplate && showCustomMessageEditor) {
			...
			properties.forEach((property, index) => {
				...
				const propertyTitle = property.title ? property.title.trim() : `Propiedad ${index + 1}`;

				// ID de la propiedad
				const propertyId = property.public_id ? property.public_id.trim() : 'Sin ID';
				...
				<div class="toggle-container">
					...
					<span class="toggle-text"
						>{showCustomMessageEditor
							...
							: 'Usar mensaje predeterminado'}</span
					>
				</div>
			</div>
				...
				<div class="message-editor-container">
					<div class="message-help">
						...
						<ul>
							...
							<li>
								<code>{'{propiedadesConUrl}'}</code> - Número de propiedades con URL disponible
							</li>
						</ul>
					</div>
					<textarea
				...
				{#each vendors as vendor}
					{#if vendor.id}
						<div
							...
							class:current-processing={isSending &&
								currentVendorIndex >= 0 &&
								selectedVendorObjects[currentVendorIndex]?.id === vendor.id &&
								isProcessingVendor}
						>
							...
							<div class="vendor-content">
											...
											<div class="property-container">
												...
												>
													...
													{#each vendorProperties[vendor.id] as prop, i}
														{#if i < 6}
															<!-- Limitar a 6 propiedades visibles -->
															<div
																class="property-card"
																style="--index: {i}; --transition-delay: {(6 - i) * 50}ms;"
															>
																{#if prop.title_image_thumb}
																	<img
																		src={prop.title_image_thumb}
													...
													{#if vendorProperties[vendor.id].length > 6}
														<div class="property-more">
															+{vendorProperties[vendor.id].length - 6} más
														</div>
													{/if}
												</div>
											</div>
								...
								{#if vendor.id && $binnaclesStore.filter((b) => b.to === vendor.id).length > 0}
									<div class="last-activity">
										Última: {formatDate(
											$binnaclesStore
												.filter((b) => b.to === vendor.id)
												.sort((a, b) => (b.date || 0) - (a.date || 0))[0]?.date
										)}
									</div>
								{/if}
							</div>
			...
			<div class="action-buttons">
				<div class="sending-progress" class:hidden={!isSending}>
					...
					<div class="progress-info">
						...
						{:else if currentVendorIndex + 1 >= selectedVendorObjects.length && currentVendorIndex >= 0}
							<span class="complete-text"
								>Proceso completado. Haz clic en "Finalizar" para terminar.</span
							>
						{:else}
							<span class="next-text"
								>Haz clic en "Enviar siguiente" para continuar con el siguiente propietario.</span
							>
						{/if}
					</div>
					<div class="progress-details">
						<span class="badge success">
							...
							{enviadosExito} enviados
						</span>
						<span class="badge warning">
							<i class="fa-solid fa-exclamation-circle"></i>
							{sinPropiedades} sin props
						</span>
				...
				>
					...
					{:else if selectedVendors.length > 0}
						Comenzar envío de reportes a {selectedVendors.length} propietario{selectedVendors.length !==
						1
							? 's'
							: ''}
					{:else}
